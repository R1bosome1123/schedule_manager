...
usermanager::usermanager()
{
    ...
    cout<<"打开了用户数据文件： ./dataset/account.dat" << endl;   
    string line;
...
void usermanager::register_user(string& user_name, string& user_password)
{
    ...
    // 将新用户信息写入文件
    ofstream account_file("./dataset/account.dat",ios::app);   // 在根目录下运行程序，用户数据文件存储在./dataset/account.dat下 覆盖写入，避免重复存储
